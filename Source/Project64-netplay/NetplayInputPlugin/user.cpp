    ...
    switch (type) {
        ...
        case SAVE_INFO: {
            for (unsigned int i = 0; i < saves.size(); i++) {
                ...
                saves[i] = new_save;
                log("[" + my_room->get_id() + "] " + "old save hash[" + old_save.sha1_data + "] is now [" + saves[i].sha1_data + "]");
            }
            for (auto& user : my_room->user_list) {
                if (user->id == id) continue;
                user->send_save_info(id, user->saves);
            }
        ...
        case SAVE_SYNC: {
            ...
            for (auto& user : my_room->user_list) {
                ...
                for (int i = 0; i < user->saves.size(); i++) {
                    auto& save = user->saves[i];
